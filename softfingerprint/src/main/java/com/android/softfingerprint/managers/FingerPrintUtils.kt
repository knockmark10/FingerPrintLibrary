...
import com.android.softfingerprint.callbacks.FingerPrintAuthCallback
import com.android.softfingerprint.callbacks.FingerPrintBasicCallback
...
class FingerPrintUtils(
...
) : PermissionCallback, FingerPrintHelperCallback {
    ...
    fun validateFingerPrint() {
        ...
        if (checkFingerPrintSensor()) {
            keyGuardManager =
                mActivity.getSystemService(Context.KEYGUARD_SERVICE) as KeyguardManager
            fingerPrintManager =
                mActivity.getSystemService(Context.FINGERPRINT_SERVICE) as FingerprintManager
            if (permissionManager.isPermissionGranted(Manifest.permission.USE_FINGERPRINT)) {
                if (fingerPrintManager.hasEnrolledFingerprints()) {
                    if (keyGuardManager.isKeyguardSecure) {
                        mBasicListener?.onFingerPrintReady()
                    } else {
                        mBasicListener?.onFingerPrintNotAvailable()
